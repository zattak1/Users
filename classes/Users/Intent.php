...
class Users_Intent extends Base_Users_Intent
{
	...
	static function capability($data = array())
	{
		...
		// Deliberately NOT cached in a static. The static cache this replaced
		// was keyed on nothing, so a second call in the same request generated
		// a fresh token, threw it away, and returned the FIRST call's
		// capability - signing away any $data the second caller passed and
		// handing back a token it had never seen. Construction is a config
		// read and an HMAC, and $time is Q::millisecondsStarted(), which is
		// constant for the request, so caching bought nothing anyway.
	 ...
	 * actions never need it - Assets/charge, the only intent action live in
	 * this deployment, is opened server-side by Assets::pay() and its token is
	 * never meant to authenticate a second device at all.
	 *
	...
	/**
	 * The fields and values we can export to clients.
	 *
	 * SECURITY: sessionId is deliberately withheld. Users/before/Q_objects.php
	 * puts this array into Q.plugins.Users.intent script data for whoever
	 * presented the token, so exporting it handed out the *originating*
	 * session's id - a longer-lived credential than the intent itself, and one
	 * that survives endTime. (Upstream Qbix reached the same conclusion after
	 * our pin; this matches its unset.) The acceptedBy fingerprint recorded by
	 * authorizeAcceptingSession() is withheld for the same reason.
	 *
	 * @method exportArray
